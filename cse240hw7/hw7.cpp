...
void executeAction(char c)
{
	...
	case 'c':		// change room type
		...
		else
		{
			...
			changeLibraryType(roomResult, type_input); //takes which room and what they want to change it to (0/1)
...
//friend function that changes priv lib in room 
void changeLibraryType(Room* room, int newLibType) {
	if (room != NULL) { //if not null
		room->lib = (libraryType)newLibType; //make int enum to show new string 
	}
}
...
void addRoom(string name_input, int no_input, libraryType type) 
{
	...
	Room* newRoom = NULL;
	if (type == hayden) {
		newRoom = new Hayden(name_input, no_input, type); //give it all 3 variables 
	}
	else if (type == noble) {
	...
	//make new container 
	Container* n = new Container(); //new node 
	n->room = newRoom; //set the room 
	n->next = NULL; //end
	...
	if (list == NULL) {
		...
		tempList->next = n; //link to the end 
	}
...
void displayList()
{
	...
	while (tempList != NULL) {
		...
		tempList = tempList->next; //keep going until all are printed 
		if (tempList != NULL) {
...
{
	int c = 0; //count
	Container* temp = list; //starts at head 
	while (temp != NULL) {
		temp = temp->next;
		c++;
	} //find count 

	ofstream myFile; //declare ofstream variable 
	myFile.open(fileName); //open file
	if (myFile.is_open()) {
		temp = list; //reset to head
		myFile << c << endl; //save count into file
		//go through list and save the rooms data 
		while (temp != NULL) {
			string typeStr; //get type as string
			if (temp->room->getLibraryType() == hayden) {
				typeStr = "Hayden";
			}
			else {
				typeStr = "Noble";
			}
			myFile << temp->room->getName() << endl; //save room name
			myFile << temp->room->getNo() << endl; //save room number
			myFile << typeStr << endl; //save room type
...
void load(string fileName)
{
	...
	int count = 0; //number of rooms
	inF >> count; //read first line (count)
	inF.ignore(); //ignore newline

	for (int i = 0; i < count; i++) { //for every room
		string name;
		int num;
		string typeStr;
		...
		inF >> num; //read number 
		inF.ignore(); //ignore newline 
		getline(inF, typeStr); //read type

		//make library type string to enum
		libraryType lt;
		if (typeStr == "Hayden") {
			lt = hayden;
		}
        ...
        //new room based on the type
		Room* nRoom;
		if (lt == hayden) {
			nRoom = new Hayden(name, num, lt);
		}
		else {
			nRoom = new Noble(name, num, lt);
		}

		//new container
		Container* nCont = new Container();
		nCont->room = nRoom;
		nCont->next = NULL;

		//add
		if (list == NULL) { //if nothing add to head 
			list = nCont;
		}
		else { //else add to end
			Container* temp = list;
			while (temp->next != NULL) {
				temp = temp->next; //search for end 
			}
			temp->next = nCont; 
		}
