...
void executeAction(char c)
{
	...
	case 'a':	// add student
		...
		scanf("%d", &IDNumber_input); //visual studio wants this to be %u cause its an unassigned int 
		flushStdIn();
...
int addSort(char* studentName_input, char* major_input, char* schoolYear_input, unsigned int IDNumber_input) // 20 points
{
	...
	while(tempList != NULL) {
		//compare destination to soirce 
		if (strcmp(tempList->studentName, studentName_input) == 0) { //returns 0 if strings are the same 
			return 0;
		}
		tempList = tempList->next; //go to next 
	 }
	//new node for new student //malloc gives as much memory as needed to struct in this case the exact size of studentRecord
	struct studentRecord* newn = (struct studentRecord*)malloc(sizeof(struct studentRecord));

	//add everything new at end of list string copy takes the location and then what to copy to that location 
	//string compare takes the location then whats being compared returns 0 if the same 
	strcpy(newn->studentName, studentName_input);
	strcpy(newn->major, major_input);
	//convert school year input into value 
	if (strcmp(schoolYear_input, "freshman") == 0)
		newn->schoolYear = freshman;
	else if (strcmp(schoolYear_input, "sophomore") == 0)
		newn->schoolYear = sophomore;
	else if (strcmp(schoolYear_input, "junior") == 0)
		newn->schoolYear = junior;
	else if (strcmp(schoolYear_input, "senior") == 0)
		newn->schoolYear = senior;

	newn->IDNumber = IDNumber_input;
	//make sure the one after is empty 
	newn->next = NULL;
	//add as first if nothing is there
	if (list == NULL) {
		list = newn;
	}
	else { //if not add to the end of the linked list 
		tempList = list;
		while (tempList->next != NULL) { //while the list has things
			tempList = tempList->next; //move on
		}
		tempList->next = newn; //once empty add the new node 
	}

	//sort alphebetically
	for (int i = 0; i < countNodes(); i++) { //go through all nodes 
		struct studentRecord* c = list; //beggining 
		while (c->next != NULL && c != NULL) {  //while a current and next node exist 
			if (strcmp(c->studentName, c->next->studentName) > 0) { //if c is greater than the next name swap
				swapNodes(c, c->next); //swap
			}
			c = c->next; //next
		}
...
{
	struct studentRecord* tempList = list;		// work on a copy of 'list'
	if (tempList == NULL) { // make sure not empty 
		return;
	}
	while(tempList != NULL) {  //while students exist
		printf("\nStudent name: %s\n", tempList->studentName);  //display name and major 
		printf("Student major: %s\n", tempList->major);
		//display year as a string
		if (tempList->schoolYear == freshman)
			printf("SchoolYear: freshman\n");
		else if (tempList->schoolYear == sophomore)
				printf("SchoolYear: sophomore\n");	
		else if (tempList->schoolYear == junior)
					printf("SchoolYear: junior\n");
		else if (tempList->schoolYear == senior)
			printf("SchoolYear: senior\n");
		//display ID 
		printf("ID number: %d\n", tempList->IDNumber);
		printf("\n");
...
int countNodes()
{
	...
	struct studentRecord* n = list; 
	while (n != NULL) { //while another node exists
		c++;  //add to counter
		n = n->next; //move on to next node
	}
	return c;	
}
...
int deleteNode(char* studentName_input) {
	...
	struct studentRecord* tempList = list;	// work on a copy of 'list'
	struct studentRecord* last = NULL; //for node before tempList nothing rn 

	if (strcmp(tempList->studentName, studentName_input) == 0) {  //see if the head is the one that needs deleting 
		list = tempList->next; //new head is next node 
		free(tempList); //free memory
		return;
	}

	last = tempList;  //move pointers to next
	tempList = tempList->next; 

	while (tempList != NULL) {
		if (strcmp(tempList->studentName, studentName_input) == 0) { //search for node to delete 
			last->next = tempList->next; //skip over tempList to remove it  
			free(tempList); //free the memory 
		}
			last = tempList; //move the pointers to next nodes
			tempList = tempList->next;
...
{
	struct studentRecord node2c = *node2; //copy of node2 for later

	strcpy(node2->studentName,node1->studentName); //copying node1 into node2
	strcpy(node2->major, node1->major);            //these have arrows because they are pointers*
	node2->schoolYear = node1->schoolYear;
	node2->IDNumber = node1->IDNumber;
	
	strcpy(node1->studentName, node2c.studentName); //node2c into node1
	strcpy(node1->major, node2c.major);             //these dont cause its just a struct variable 
	node1->schoolYear = node2c.schoolYear;
	node1->IDNumber = node2c.IDNumber;
