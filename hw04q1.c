...
// Each student record has this information: student's name, major, school year of
// student, ID.
...
int count = 0; // the number of students
...
// Sort can be implemented as a different method and called it every you add a
// record.
// Use a temp struct (already declared) if you need to swap two structs in your
// logic.
// Do not allow the student to be added to the list if they already exist in the
...
int addSort(char* studentName_input, char* major_input, char* schoolYear_input, unsigned int studentID_input){
	...
	//check is students exists already
	for (int i = 0; i < count; i++) {
		//strcmp is a c function that compares two strings - 0 if things are the same
		if(strcmp(list[i].studentName, studentName_input) == 0) {
			return 0;
		}
	}

	//add student at end of list
	//strcpy is used to copy a string from a source to a destination (start with destination)
	strcpy(list[count].studentName, studentName_input);
	strcpy(list[count].major, major_input);
	//convert schoolyear input into actual value 
	if(strcmp(schoolYear_input, "freshman") == 0) 
		list[count].schoolYear = freshman;
	
	else if(strcmp(schoolYear_input, "sophomore") == 0) 
		list[count].schoolYear = sophomore;
	
	else if(strcmp(schoolYear_input, "junior") == 0) 
		list[count].schoolYear = junior;
	
	else if(strcmp(schoolYear_input, "senior") == 0) 
		list[count].schoolYear = senior;
	...
	//sort list
	for (int i = 0; i < count - 1; i++) { //goes from 1st student to 2nd to last 
		for (int j = i + 1; j < count; j++) { //goes from student after i to last 
			if (strcmp(list[i].studentName, list[j].studentName) > 0) { //returns > 0 if first string is greater than second
				//swap
				struct studentRecord temp = list[i];
				list[i] = list[j];
				list[j] = temp;
			}
			}
...
// expected output screenshots in the PDF file.
...
void display()
{
	for (int i = 0; i < count; i++) {
		...
		else if (list[i].schoolYear == senior)
...
void load(char* fileName)
{
	...
	file = fopen(fileName, "rb"); //rb is for reading binary file
	if (file == NULL) {
		printf("%s not found\n", fileName);
		return;
		}
	...
	fread(&count, sizeof(count), 1, file); //&count: the & gives the address of count variable
										   //sizeof(count):tells fread how many bytes to read for one element 
										   //1: number of elements to read (count itself is one thing)
	//loop through each student and read data in order it was saved
	for (int i = 0; i < count; i++) {
		fread(list[i].studentName, sizeof(list[i].studentName), 1, file);
		fread(list[i].major, sizeof(list[i].major), 1, file);        //strings so no & at the front

		fread(&schoolYearValue, sizeof(schoolYearValue), 1, file);   //&schoolYearValue: fread needs an address to store data 
		if (schoolYearValue == 0)                                    //int syv is just an int so need to use & to give it an address to put data
